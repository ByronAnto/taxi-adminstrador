...
/// cada envío. Envía por TOKENS a los usuarios activos/paymentPending del rol,
/// con `.select` (menos egress) y purga de tokens muertos (`_sendMulticastAndPrune`).
///
/// NOTA: la app YA suscribe a cada usuario al topic `role_<su rol>` al iniciar
/// sesión (convención: role_conductor/role_admin/role_operadora). Cuando el 100%
/// del parque esté en el APK que suscribe, se puede migrar este broadcast a
/// topics (1 send por rol, sin leer Firestore) — beneficio a escala. POR AHORA
/// se mantiene por tokens para NO perder entrega con apps que aún no suscriben.
